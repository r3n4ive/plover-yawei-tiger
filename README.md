...
提交前可运行 `python tools/rime/sync_dictionary.py --check` 检查是否同步。
...
亚伟的对称拼音叠音 chord 也会保留两个音节。例如 `BA-BA` 是一个 chord，含义是重复的
`ba-ba`，插件会把它送给 Rime 作为 `ba'ba`，不会误缩成一个 `ba`；因此无论候选窗口是否
显示，叠音都不会丢失第二个音节。对称虎码辅码仍按一个形码处理，不会被错误拆成两个音节。
...
当前实现已达到第一阶段验收标准：中文允许重码、有可见候选框、支持选择和提交，英文现有输入不受影响；librime 或候选 UI 异常时，Plover 会记录错误并回退到原有流程。
...
1. 已完成 Yawei 硬件驱动和当前 Plover 词库的稳定性测试。
2. 已完成 stroke 到候选引擎的桥接、隔离 librime runtime 和数据目录。
3. 已完成中文候选框、选择键和中英文模式切换。
4. 下一步：用真实亚伟输入记录重码、选择次数和延迟，调整词库与候选排序。
